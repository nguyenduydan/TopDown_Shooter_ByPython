...
health_potion_sound.set_volume(0.2)
...
game_active = True  # Trạng thái hoạt động của trò chơi
beat_game = False   # Đánh dấu xem người chơi đã chiến thắng trò chơi hay chưa
current_time = 0    # Thời gian hiện tại trong trò chơi
level_over_time = 0 # Thời gian kết thúc cấp độ hoặc giai đoạn trong trò chơi
ready_to_spawn = False  # Xác định xem có sẵn sàng để xuất hiện các đối tượng trong trò chơi hay không
display_countdown_time = False  # Hiển thị đếm ngược trước khi trò chơi bắt đầu hoặc trước sự kiện quan trọng
first_level = True  # Xác định xem trò chơi hiện tại là cấp độ đầu tiên hay không

 # Hàm kiểm tra va chạm giữa hai sprite
def hitbox_collide(sprite1, sprite2):
    return sprite1.base_zombie_rect.colliderect(sprite2.rect)

#lớp quản lý nhât vật người chơi
class Player(pygame.sprite.Sprite):
    ...
    #Hàm quay nhân vật theo con trỏ chuột
    ...
    #hàm tăng máu khi nhặt được bình máu
...
class Enemy(pygame.sprite.Sprite):
    ...
    def check_alive(self): #Kiểm tra xem enemy còn sống hay không
        if self.health <= 0:
            self.alive = False
    ...
    def roam(self):
        # Hàm này xử lý việc di chuyển ngẫu nhiên của enemy khi không phát hiện người chơi
    ...
    #Hàm random ra vị trí ngẫu nhiên
    ...
    def hunt_player(self):
        # Hàm này xử lý việc enemy săn theo người chơi
        if self.velocity.x > 0:
    ...
    #hàm import vào các sprites ảnh
    ...
    def update(self):
        # Hàm cập nhật trạng thái của enemy
        ...
        if self.alive:
            ...
            if self.get_vector_distance(pygame.math.Vector2(player.base_player_rect.center),
                ...
                if self.get_vector_distance(pygame.math.Vector2(player.base_player_rect.center), pygame.math.Vector2(self.base_zombie_rect.center)) < 700:
                    self.current_index = self.animate(self.current_index, self.roam_animation_speed, self.animations["hunt"], "idle")
...
#Lớp quản lý giao diện
class UI():
    def __init__(self):
        # Khởi tạo thông tin về thanh máu và một số thông tin UI khác
    ...
    def display_health_text(self):
         # Hiển thị văn bản thông tin về máu
    ...
    def display_wave_text(self):
        # Hiển thị thông tin về sóng (wave)
    ...
    #Hiển thị thời gian giữa các wave
    ...
    #Hiển thị số lượng enemy
    def display_enemy_count(self):
         # Hiển thị thông tin về số lượng enemy còn lại trong wave
    ...
    #hiển thị tốc dộ khung hình
    def display_fps(self, clock):
        # Hiển thị fps
        fps_surface = font.render(f"FPS: {int(clock.get_fps())}", True, WHITE)
        fps_rect = fps_surface.get_rect(topleft=(10,690))
...
class Bullet(pygame.sprite.Sprite):
    ...
    #hàm tính vận tốc cũng như đường đi của viên đạn
    ...
    #hàm va chạm giữa đạn với tường và quái vật
...
class Item(pygame.sprite.Sprite):
    ...
    #kiểm tra nhân vật đã nhặt chưa và tính toán đồng xu tăng
    def check_player_collision(self):
...
class GameLevel(pygame.sprite.Group):
    ...
    #Hàm tạo ra bản đồ trong trò chơi
    def create_map(self):
    ...
    #import các maps dựa vào đọc file csv
    ...
    #vẽ các máp đó ra dựa vào vị trí được thiết lập sẵn
...
class Tile(pygame.sprite.Sprite):
    def __init__(self, pos, groups, type, unique_id):
        ...
        if type == "boundary":
            ...
            # Xác định hình ảnh (self.image) cho sprite dựa trên type và unique_id.
            if unique_id == "19":
         ...
         # Xác định rect cho sprite, với vị trí ở góc trái trên là pos.
...
while True:
    ...
    keys = pygame.key.get_pressed() #nhận sự khiện bấm nút
    for event in pygame.event.get(): #các sự kiện trong game
        ...
        if not game_active and keys[pygame.K_p]:
            ...
            game_active = True #game được chạy
            game_stats["current_wave"] = 1 #wave đầu tiên
            ...
            game_stats["coins"] = 0 #số lượng đồng xu thu thập được
            #hàm spawn
            game_level.spawn_enemies()
            game_level.spawn_health_potions()

    if game_active: #khi game chạy
